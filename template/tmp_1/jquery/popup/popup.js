...
function loadPopupForgot(){
	//Will only be loaded if the status is 0
	if(popupForgot==0){
		$(".backgroundPopupForgot").css({"opacity": "0.8"});
		$(".backgroundPopupForgot").fadeIn("slow");
		$(".pop_forgot").fadeIn("slow");
		popupForgot = 1;
...
function disablePopupForgot(){
	//Will only disable if status is 1
	if(popupForgot==1){
		$(".backgroundPopupForgot").fadeOut("slow");
		$(".pop_forgot").fadeOut("slow");
		popupForgot = 0;
	}
}
...
function centerPopupForgot(){
	var windowWidth = document.documentElement.clientWidth;
	var windowHeight = document.documentElement.clientHeight;
	var popupHeight = $(".pop_forgot").height();
	var popupWidth = $(".pop_forgot").width();
	$(".pop_forgot").css({
		"position": "absolute",
		"top": windowHeight/2-popupHeight/2,
		"left": windowWidth/2-popupWidth/2
	...
	//this is needed for ie6
	$(".backgroundPopupForgot").css({ "height": windowHeight });
}
...
$(document).ready(function(){
	...
	//Close popup by clicking the x
	$(".popupCloseForgot").click(function(){ disablePopupForgot(); });
	//Close popup by clicking outside the box
	$(".backgroundPopupForgot").click(function(){ disablePopupForgot(); });
	//Close popup by clicking escape
	$(document).keypress(function(e){
		if(e.keyCode==27 && popupForgot==1){
			disablePopupForgot();
		}
	});
});
