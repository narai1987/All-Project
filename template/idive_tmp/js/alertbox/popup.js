...
//for rating
var popupStatus_rating = 0;
//for coupon
var popupStatus_coupon = 0;
...
//for rating
function loadPopup_rating(){
	//loads popup only if it is disabled
	if(popupStatus_rating==0){
		$("#backgroundPopup_rating").css({
			"opacity": "0.7"
		});
		$("#backgroundPopup_rating").fadeIn("slow");
		$("#popupContact_rating").fadeIn("slow");
		popupStatus_rating = 1;
	}
}
//for coupon
function loadPopup_coupon(){
	//loads popup only if it is disabled
	if(popupStatus_coupon==0){
		$("#backgroundPopup_coupon").css({
			"opacity": "0.7"
		});
		$("#backgroundPopup_coupon").fadeIn("slow");
		$("#popupContact_coupon").fadeIn("slow");
		popupStatus_coupon = 1;
	}
}
...
//for rating
function disablePopup_rating(){
	//disables popup only if it is enabled
	if(popupStatus_rating==1){
		$("#backgroundPopup_rating").fadeOut("slow");
		$("#popupContact_rating").fadeOut("slow");
		popupStatus_rating = 0;
	}
}
//for coupon
function disablePopup_coupon(){
	//disables popup only if it is enabled
	if(popupStatus_coupon==1){
		$("#backgroundPopup_coupon").fadeOut("slow");
		$("#popupContact_coupon").fadeOut("slow");
		popupStatus_coupon = 0;
	}
}
...
//for rating
function centerPopup_rating(){
	//request data for centering
	var windowWidth = document.documentElement.clientWidth;
	var windowHeight = document.documentElement.clientHeight;
	var popupHeight_rating = $("#popupContact_rating").height();
	var popupWidth_rating = $("#popupContact_rating").width();
	//centering
	$("#popupContact_rating").css({
		"position": "absolute",
		"top": windowHeight/2-popupHeight_rating/2,
		"left": windowWidth/2-popupWidth_rating/2
	});
	//only need force for IE6
	$("#backgroundPopup_rating").css({
		"height": windowHeight
...
//for coupon
function centerPopup_coupon(){
	//request data for centering
	var windowWidth = document.documentElement.clientWidth;
	var windowHeight = document.documentElement.clientHeight;
	var popupHeight_rating = $("#popupContact_coupon").height();
	var popupWidth_rating = $("#popupContact_coupon").width();
	//centering
	$("#popupContact_coupon").css({
		"position": "absolute",
		"top": windowHeight/2-popupHeight_rating/2,
		"left": windowWidth/2-popupWidth_rating/2
	});
...
$(document).ready(function(){
	...
	//for rating
	$(document).keypress(function(e){
		if(e.keyCode==27 && popupStatus_rating==1){
			disablePopup_rating();
		}
	});
	//for coupon
	$(document).keypress(function(e){
		if(e.keyCode==27 && popupStatus_coupon==1){
			disablePopup_coupon();
		}
	});
