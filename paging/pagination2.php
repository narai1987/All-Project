...
function paginate_three($reload, $page, $tpages, $adjacents,$maxpage=NULL) {
	...
	elseif($page==2) {
		$out.= "<a onclick=paging('".$reload."') >" . $prevlabel . "</a>\n";
		//$out.= "<a onclick='paging(".$reload.")' href=\"" . $reload . "\">" . $prevlabel . "</a>\n";
	}
	else {
		//$out.= "<a href=\"" . $reload . "&amp;page=" . ($page-1) . "\">" . $prevlabel . "</a>\n";
		$out.= "<a onclick=paging('".$reload. "&amp;page=" . ($page-1) ."')  >" . $prevlabel . "</a>\n";
	}
	
	// first
	if($page>($adjacents+1)) {
		$out.= "<a onclick=paging('') >1</a>\n";
	...
	for($i=$pmin; $i<=$pmax; $i++) {
		...
		elseif($i==1) {
			$out.= "<a onclick=paging('" . $reload . "')>" . $i . "</a>\n";
		}
		else {
			$out.= "<a onclick=paging('".$reload."&amp;page=". $i."')>" . $i . "</a>\n";
		}
	}
	...
	if($page<($tpages-$adjacents)) {
		$out .= "<a onclick=paging('".$reload."&amp;page=". $tpages."')></a>\n";
	}
	
	// next
	//echo $maxpage; 
	if($page<$maxpage) {
		$out .= "<a onclick=paging('".$reload."&amp;page=".($page+1)."')>" . $nextlabel . "</a>\n";
	}
	else {
