<?php
	...
	class LanguageClass extends DbAccess {
		...
		function addLanguage() {
			...
			echo $query = "select * from languages where id = '".$_REQUEST['id']."'";
			$this->query($query);
			$arrData['languages'] = $this->fetchArray();
		...
		function save() {
			...
			$query = "UPDATE languages SET content='".$content."', date_time='".date("Y-m-d H:i:s")."' WHERE id = '".$_REQUEST['id']."'";
			$this->Query($query);
			$this->Execute();
			...
			$query = "INSERT INTO `languages` (`content`, `date_time`,`status`) VALUES ('".$content."','".date("Y-m-d H:i:s")."','1')";
			$this->Query($query);
			$this->Execute();
