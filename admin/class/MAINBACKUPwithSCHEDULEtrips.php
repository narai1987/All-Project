	...
	class tripClass extends DbAccess {
		...
		function save() {
			...
			if(!$_REQUEST['id'] and ($boat_id!='')) {	
				...
				$query = "UPDATE `trips` SET `boat_id` = '".$boat_id."', `trip_type_id` = '".$trip_type_id."', `country_id` = '".$country_id."', `destination_country_id` = '".$destination_country_id."', `origin_id` = '".$origin_id."', `destination_id` = '".$destination_id."', `start_date` = '".$start_date."', `end_date` = '".$end_date."', `no_of_days` = '".$no_of_days."', `no_of_nights` = '".$no_of_nights."', `trip_price` = '".$trip_price."', `price_for_diver` = '".$price_for_diver."', `price_for_kids` = '".$price_for_kids."', `modified_date` = '".$modified_date."' WHERE id = '".$currentID."'";
				$this->Query($query);
				$this->Execute();
		...
		function addschedule() {
			...
			if($_REQUEST['scheduleId']) {
				...
				foreach($trip_beverages as $trip_food) {
			...
			if($_REQUEST['trip_id']) {
				if($_REQUEST['schedule_id']) {
					...
					foreach($equipments as $equipment) {
						$chk_eq = "chk_eq_".$equipment['id'];
						if($_REQUEST[$chk_eq]) {
							$eqtype = "eqtype".$equipment['id'];
							$eqprice_ = "eqprice_".$equipment['id'];
							$query_eq ="INSERT INTO trip_equipments(`trip_id`,`trip_schedule_id`,`equipment_id`,`eq_value`,`eq_type`,`eq_status`) VALUES( '".$_REQUEST['trip_id']."','".$_REQUEST['schedule_id']."','".$equipment['id']."','".$_REQUEST[$eqprice_ ]."','".$_REQUEST[$eqtype]."','1')";
							$this->Query($query_eq);
							$this->Execute();
						}
					}
					...
					foreach($beverages as $beverage) {
						$chk_bv = "chk_bv_".$beverage['id'];
						if($_REQUEST[$chk_bv]) {
							$bvtype = "bvtype".$beverage['id'];
							$bvprice_ = "bvprice_".$beverage['id'];
							$query_bv ="INSERT INTO trip_beverages(`trip_id`,`trip_schedule_id`,`beverage_id`,`eq_value`,`eq_type`,`eq_status`) VALUES( '".$_REQUEST['trip_id']."','".$_REQUEST['schedule_id']."','".$beverage['id']."','".$_REQUEST[$bvprice_ ]."','".$_REQUEST[$bvtype]."','1')";
							$this->Query($query_bv);
							$this->Execute();
						}
					}
					...
					foreach($foods as $food) {
						$chk_fd = "chk_fd_".$food['id'];
						if($_REQUEST[$chk_fd]) {
							$fdtype = "fdtype".$food['id'];
							$fdprice_ = "fdprice_".$food['id'];
							$query_fd ="INSERT INTO trip_foods(`trip_id`,`trip_schedule_id`,`food_id`,`eq_value`,`eq_type`,`eq_status`) VALUES( '".$_REQUEST['trip_id']."','".$_REQUEST['schedule_id']."','".$food['id']."','".$_REQUEST[$fdprice_ ]."','".$_REQUEST[$fdtype]."','1')";
							$this->Query($query_fd);
							$this->Execute();
						}
					}
					...
					foreach($equipments as $equipment) {
						...
						if($_REQUEST[$chk_eq]) {
							$eqtype = "eqtype".$equipment['id'];
							$eqprice_ = "eqprice_".$equipment['id'];
							$query_trip_eq ="INSERT INTO trip_equipments(`trip_id`,`trip_schedule_id`,`equipment_id`,`eq_value`,`eq_type`,`eq_status`) VALUES( '".$_REQUEST['trip_id']."','".$trip_shedule_id."','".$equipment['id']."','".$_REQUEST[$eqprice_]."','".$_REQUEST[$eqtype]."','1')";
							$this->Query($query_trip_eq);
							$this->Execute();
						}
					}
					...
					foreach($beverages as $beverage) {
						...
						if($_REQUEST[$chk_bv]) {
							$bvtype = "bvtype".$beverage['id'];
							$bvprice_ = "bvprice_".$beverage['id'];
							$query_trip_bv ="INSERT INTO trip_beverages(`trip_id`,`trip_schedule_id`,`beverage_id`,`eq_value`,`eq_type`,`eq_status`) VALUES( '".$_REQUEST['trip_id']."','".$trip_shedule_id."','".$beverage['id']."','".$_REQUEST[$bvprice_]."','".$_REQUEST[$bvtype]."','1')";
							$this->Query($query_trip_bv);
							$this->Execute();
						}
					}
					...
					foreach($foods as $food) {
						...
						if($_REQUEST[$chk_fd]) {
							$fdtype = "fdtype".$food['id'];
							$fdprice_ = "fdprice_".$food['id'];
							$query_trip_fd ="INSERT INTO trip_foods(`trip_id`,`trip_schedule_id`,`food_id`,`eq_value`,`eq_type`,`eq_status`) VALUES( '".$_REQUEST['trip_id']."','".$trip_shedule_id."','".$food['id']."','".$_REQUEST[$fdprice_]."','".$_REQUEST[$fdtype]."','1')";
							$this->Query($query_trip_fd);
							$this->Execute();
						}
					}
		...
		function itinerary() {
			...
			if($tripItineraryID) {
				...
				if($this->countItinerary($scheduleID,$tripID)) {
					...
					$q = "select * from trip_itinerary ti JOIN trip_schedules ts ON ti.trip_schedule_id = ts.id  where ti.id = (select max(t.id) from trip_itinerary t where t.trip_id = '".$tripID."' and t.trip_schedule_id = '".$scheduleID."')  ORDER BY id DESC ";
					$this->Query($q);
		...
		function itinerary_old() {
			...
			if($tripItineraryID) {
				...
				if(count($max_id)) {
					...
					//######################################################################################################/////
					echo $q_no_itinerary = "select tit.*, c.country_id from trip_itinerary tit JOIN cities c ON tit.arrival_place = c.id JOIN countries cntr ON c.country_id = cntr.id where tit.id = '".$tripItineraryID."' ";
		...
		function saveItinerary() {
			...
			if($itineraryId) {
				...
				if($str) {
					...
					$query = "UPDATE trip_itinerary_details SET ".$str." WHERE trip_id = '".$tripID."' and trip_schedule_id = '".$scheduleID."' and id = '".$itineraryId."'";
					$this->Query($query);
					$this->Execute();
				}
				...
				$query = "INSERT INTO trip_itinerary(trip_id,trip_schedule_id,deprture_country_id,arrival_country_id,departure_place,arrival_place,departure_time,arrival_time,itinerary_date,no_of_dive,status) VALUES('".$tripID."','".$scheduleID."','".$deprture_country_id."','".$arrival_country_id."','".$departure_place."','".$arrival_place."','".$departure_time."','".$arrival_time."','".$itinerary_date."','".$no_of_dive."','1')";
			
				$this->Query($query);
				$this->Execute();
				
				$querydeatil = "INSERT INTO trip_itinerary_details (trip_itinerary_id,language_id,night_schedule,detail,status) VALUES('".mysql_insert_id()."','".$_SESSION['language_id']."','".$_REQUEST['night_schedule1']."','".$_REQUEST['detail1']."','1')";
		...
		function saveOldItinerary() {
			...
			foreach($data as $result) {
				$str .= $str?",( '".$tripId."', '".$scheduleId."', '".$result['deprture_country_id']."', '".$result['arrival_country_id']."', '".$result['departure_place']."', '".$result['arrival_place']."', '".$result['departure_time']."', '".$result['arrival_time']."', '".$result['dive_time']."', '".$result['date_days']."', '".$result['no_of_dive']."', '".$result['detail']."', 1)":"( '".$tripId."', '".$scheduleId."', '".$result['deprture_country_id']."', '".$result['arrival_country_id']."', '".$result['departure_place']."', '".$result['arrival_place']."', '".$result['departure_time']."', '".$result['arrival_time']."', '".$result['dive_time']."', '".$result['date_days']."', '".$result['no_of_dive']."', '".$result['detail']."', 1)";
			}
			echo $query = "INSERT INTO `trip_itinerary` (`trip_id`, `trip_schedule_id`, `deprture_country_id`, `arrival_country_id`, `departure_place`, `arrival_place`, `departure_time`, `arrival_time`, `dive_time`, `date_days`, `no_of_dive`, `detail`, `status`) VALUES ".$str;
			$this->Query($query);
			$this->Execute();
