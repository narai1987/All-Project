<?php
	...
	class boatClass extends DbAccess {
		...
		function addnew() {
				...
				/*Getting Boat Technical Options*/
				 $query_techop ="SELECT * FROM  boat_technical WHERE language_id = '".$_SESSION['language_id']."'";
				$this->Query($query_techop);
				$techOptions = $this->fetchArray();
				/*Getting Boat Technical Options*/
				
				/*Getting Cockpit Features Options*/
				 $query_cockpit ="SELECT * FROM  boat_cockpit WHERE language_id = '".$_SESSION['language_id']."'";
				$this->Query($query_cockpit);
				$cockpitFeatures = $this->fetchArray();
				/*Getting Cockpit Features Options*/
				
				/*Getting Helm Features Options*/
				 $query_helm ="SELECT * FROM  boat_helm WHERE language_id = '".$_SESSION['language_id']."'";
				$this->Query($query_helm);
				$helmFeatures = $this->fetchArray();
				/*Getting Helm Features Options*/
				
				/*Getting Hull&Deck Features Options*/
				 $query_hull ="SELECT * FROM  boat_hulldeck WHERE language_id = '".$_SESSION['language_id']."'";
				$this->Query($query_hull);
				$hullDecks = $this->fetchArray();
				/*Getting Hull&Deck Features Options*/
				
				/*Getting Engine & Technical Features Options*/
				 $query_engine ="SELECT * FROM   boat_engine WHERE language_id = '".$_SESSION['language_id']."'";
				$this->Query($query_engine);
				$engineFeatures = $this->fetchArray();
				/*Getting Engine & Technical Features Options*/
				
				/*Getting Engine Power Options*/
				 $query_enginePower ="SELECT * FROM   boat_enginepower WHERE language_id = '".$_SESSION['language_id']."'";
				$this->Query($query_enginePower);
				$enginePowers = $this->fetchArray();
				/*Getting Engine Power Options*/
		...
		function save() {
			...
			$technicalOptions = $_REQUEST['technicalOption'];
			$cockpits = $_REQUEST['cockpit'];
			$helms = $_REQUEST['helm'];
			$hullDecks = $_REQUEST['hullDeck'];
			$enginePowers = $_REQUEST['enginePower'];
			$engineFeatures = $_REQUEST['engineFeature'];
			
				if($technicalOptions):
					$i = 0;
					foreach($technicalOptions AS $key=>$val):
						if($i == 0){
						$technicalOptionValues .= $val;
						}else{
						$technicalOptionValues .= ",".$val;
						} 
					$i++;
					endforeach;
				$insertVarBoatFeatureColumn .= "`boat_technical` ,";
				$insertVarBoatFeatureValues .= "'".$technicalOptionValues."' ,";
				$updateVarBoatFeatures .= "`boat_technical` = '".$technicalOptionValues."' ,";
				endif;
				
				if($cockpits):
					$i = 0;
					foreach($cockpits AS $key=>$val):
						if($i == 0){
						$cockpitsValues .= $val;
						}else{
						$cockpitsValues .= ",".$val;
					...
					$i++;
					endforeach;
				$insertVarBoatFeatureColumn .= "`boat_cockpit` ,";
				$insertVarBoatFeatureValues .= "'".$cockpitsValues."' ,";
				$updateVarBoatFeatures .= "`boat_cockpit` = '".$cockpitsValues."' ,";
				endif;
				
				if($helms):
					$i = 0;
					foreach($helms AS $key=>$val):
						if($i == 0){
						$helmsValues .= $val;
						}else{
						$helmsValues .= ",".$val;
					...
					$i++;
					endforeach;
				$insertVarBoatFeatureColumn .= "`boat_helm` ,";
				$insertVarBoatFeatureValues .= "'".$helmsValues."' ,";
				$updateVarBoatFeatures .= "`boat_helm` = '".$helmsValues."' ,";
				endif;
				
				if($hullDecks):
					$i = 0;
					foreach($hullDecks AS $key=>$val):
						if($i == 0){
						$hullDecksValues .= $val;
						}else{
						$hullDecksValues .= ",".$val;
					...
					$i++;
					endforeach;
				$insertVarBoatFeatureColumn .= "`boat_hulldeck` ,";
				$insertVarBoatFeatureValues .= "'".$hullDecksValues."' ,";
				$updateVarBoatFeatures .= "`boat_hulldeck` = '".$hullDecksValues."' ,";
				endif;
				
				if($enginePowers):
					$i = 0;
					foreach($enginePowers AS $key=>$val):
						if($i == 0){
						$enginePowersValues .= $val;
						}else{
						$enginePowersValues .= ",".$val;
						} 
					$i++;
					endforeach;
				$insertVarBoatFeatureColumn .= "`boat_enginepower` ,";
				$insertVarBoatFeatureValues .= "'".$enginePowersValues."' ,";
				$updateVarBoatFeatures .= "`boat_enginepower` = '".$enginePowersValues."' ,";
				endif;
				
				if($engineFeatures):
					$i = 0;
					foreach($engineFeatures AS $key=>$val):
						if($i == 0){
						$engineFeaturesValues .= $val;
						}else{
						$engineFeaturesValues .= ",".$val;
						} 
					$i++;
					endforeach;
				$insertVarBoatFeatureColumn .= "`boat_engine` ,";
				$insertVarBoatFeatureValues .= "'".$engineFeaturesValues."' ,";
				$updateVarBoatFeatures .= "`boat_engine` = '".$engineFeaturesValues."' ,";
				endif;
			...
			$men_capacity = mysql_real_escape_string($_REQUEST['men_capacity']);
			...
			if(!$_REQUEST['id']) {	
				...
				$query = "INSERT INTO `boats` (".$insertVarBoatFeatureColumn."`operator_id`,`boat_type_id`, `company_id`, `country_id`, `company_branch_id`, `established`, `latest_overhall`, `hull_type`, `hull_material`, `boat_width`, `boat_range`, `fresh_water_capacity`, `gray_water_capacity`, `black_water_capacity`, `boat_price`, `boat_length`, `boat_beam`, `fuel_capacity`, `men_capacity`, `upper_deck`, `main_deck`, `lower_deck`, `draft_drive_up_high_trim`, `draft_drive_down`, `deadrise`, `approx_dry_weight`, `estimated_height_on_trailer_top_windshield`, `boat_height_windshield_to_keel`, `bridge_clearance_top_up`, `bridge_clearance_top_down`, `cockpit_depth_helm`, `cockpit_storage`, `child_discount1`, `child_discount2`, `date_time`, `status`) VALUES (".$insertVarBoatFeatureValues."'".$operator_id."','".$boat_type_id."', '".$company_id."', '".$country_id."', '".$company_branch_id."', '".$established."', '".$latest_overhall."', '".$hull_type."', '".$hull_material."', '".$boat_width."', '".$boat_range."', '".$fresh_water_capacity."', '".$gray_water_capacity."', '".$black_water_capacity."', '".$boat_price."', '".$boat_length."', '".$boat_beam."', '".$fuel_capacity."', '".$men_capacity."', '".$upper_deck."', '".$main_deck."', '".$lower_deck."', '".$draft_drive_up_high_trim."', '".$draft_drive_down."', '".$deadrise."', '".$approx_dry_weight."', '".$estimated_height_on_trailer_top_windshield."', '".$boat_height_windshield_to_keel."', '".$bridge_clearance_top_up."', '".$bridge_clearance_top_down."', '".$cockpit_depth_helm."', '".$cockpit_storage."', '".$child_discount1."', '".$child_discount2."', '".$date_time."', '".$status."')";
				$this->Query($query);
				$this->Execute();
