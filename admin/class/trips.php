<?php
	...
	class tripClass extends DbAccess {
		...
		function fillBoats(){
				...
				/*Getting Boat Price Range*/
				$query_range_low = "SELECT value FROM confic WHERE title = 'trip_low_price_range'";
				$this->Query($query_range_low);
				$boatLowPriceRange = $this->fetchArray();
				
				$query_range_high = "SELECT value FROM confic WHERE title = 'trip_high_price_range'";
				$this->Query($query_range_high);
				$boatHighPriceRange = $this->fetchArray();
				/*Getting Boat Price Range*/
			
			if($_REQUEST['boatPriceRange'] == 'low'){
				$boatPrce = " AND b.boat_price < ".$boatLowPriceRange[0]['value']."";
				}elseif($_REQUEST['boatPriceRange'] == 'mid'){
				$boatPrce = " AND b.boat_price >= ".$boatLowPriceRange[0]['value']." AND boat_price <= ".$boatHighPriceRange[0]['value'];
				}elseif($_REQUEST['boatPriceRange'] == 'high'){
				$boatPrce = " AND b.boat_price > ".$boatHighPriceRange[0]['value']."";
				}else{
				$boatPrce = "";
				}
			
			
			$query_comp ="SELECT bs.boat_name,bs.boat_id,b.boat_type_id FROM "
			. " boatspecifications bs JOIN boats b ON b.id = bs.boat_id WHERE"
			. " language_id = '".$_SESSION['language_id']."' AND b.boat_type_id = '".$_REQUEST['triptype']."'".$boatPrce."";
				$this->Query($query_comp);
				$boats = $this->fetchArray();
					require_once("views/".$this->name."/fillBoats.php");		
		}
		...
		function show($view) {	
			...
			$query ="SELECT trips.id AS id, trip_specifications.trip_title,trips.status from trips JOIN trip_specifications ON trips.id = trip_specifications.trip_id  WHERE trip_specifications.language_id = '".$_SESSION['language_id']."' ORDER BY trips.modified_date desc ";
			$this->Query($query);
		...
		/* Paging end here */
			...
			$trips = $this->fetchArray();
				if($trips=='' and $tdata > 0){
				$page   = $_REQUEST['page'] - 1;
				
			$query ="SELECT trips.id AS id, trip_specifications.trip_title,trips.status from trips JOIN trip_specifications ON trips.id = trip_specifications.trip_id  WHERE trip_specifications.language_id = '".$_SESSION['language_id']."' LIMIT ".(($page-1)*$tpages).",".$tpages;
			$this->Query($query);
			$trips = $this->fetchArray();	
			}
		...
		/*CHECK IF THIS TRIP IS BOOKED BY CUSTOMERS OR NOT IF FOUND THEN NOTICE THE ADMIN AND SEND EMAIL REGARDING CHANGES ON TRIP TO ALL CUSTOMERS*/	
		
			 						$isBookedQuery = "SELECT b.id
									FROM bookings b
									JOIN trip_schedules sc ON sc.id = b.trip_schedule_id
									WHERE sc.start_trip_datetime >=  '".date("Y-m-d")."'
									AND b.trip_id =  '".$_REQUEST['id']."'";
							
					$this->Query($isBookedQuery);
					$isBooking = $this->fetchArray();
		...
		/*CHECK IF THIS TRIP IS BOOKED BY CUSTOMERS OR NOT IF FOUND THEN NOTICE THE ADMIN AND SEND EMAIL REGARDING CHANGES ON TRIP TO ALL CUSTOMERS*/
			...
			$query_trip_eq ="SELECT * FROM  `trip_equipments` WHERE trip_schedule_id = '".$scheduleid[0]['id']."' AND trip_id =  '".$_REQUEST['id']."'";				
				...
				/*Getting Type Of Gases*/
				$query_trip_gas ="SELECT * FROM  gas_type WHERE 1";
				$this->Query($query_trip_gas);
				$gas_types = $dataGas = $this->fetchArray();
				
				foreach($dataGas as $gasval) {
					$Gquery_all ="SELECT * FROM  `trip_gas_type` WHERE gas_type_id = '".$gasval['id']."' AND `trip_id` = '".$_REQUEST['id']."'";
					$this->Query($Gquery_all);
					$GallContents = $this->fetchArray();
					foreach($GallContents as $GallContent){
						$results['gasdata'][$gasval['id']] = $GallContent;
					}
				}
		...
		$getSchedulteDateQ = "SELECT start_trip_datetime,end_trip_datetime,trip_price FROM trip_schedules WHERE id = '".$scheduleid[0]['id']."'";
			...
			/*Getting BOATS Start*/
				...
				if($results['common'][0]->boat_price_range == 'low'){
				$boatPrce = " AND b.boat_price < ".$boatLowPriceRange[0]['value']."";
				}elseif($results['common'][0]->boat_price_range == 'mid'){
				$boatPrce = " AND b.boat_price >= ".$boatLowPriceRange[0]['value']." AND boat_price <= ".$boatHighPriceRange[0]['value'];
				}elseif($results['common'][0]->boat_price_range == 'high'){
				$boatPrce = " AND b.boat_price > ".$boatHighPriceRange[0]['value']."";
				}else{
				$boatPrce = "";
				}
				...
				$query_comp ="SELECT bs.boat_name,bs.boat_id,b.boat_type_id FROM"
				. " boatspecifications bs JOIN boats b ON b.id = bs.boat_id WHERE "
				. " language_id = '".$_SESSION['language_id']."' AND b.boat_type_id = '".$results['common'][0]->trip_type_id."'".$boatPrce."";
				$this->Query($query_comp);
				$boats = $this->fetchArray();
				/*Getting BOATS Close*/
			...
			$results['common'][0]->trip_price = $scheduleDates[0]['trip_price'];
				...
				/*Getting Gases*/
				$query_gas ="SELECT * FROM  gas_type WHERE 1";
				$this->Query($query_gas);
				$gas_types = $this->fetchArray();
				/*Getting Gases*/
		...
		function getBoatPrice(){
			$query = "SELECT boat_price FROM boats WHERE id = '".$_REQUEST['boat_id']."' ";
			$this->Query($query);
			$results = $this->fetchArray();
			echo $results[0]['boat_price'];
	...
	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////			
		function boatDetailBACKUP(){
			...
			require_once("views/".$this->name."/viewBoatDetail.php"); 
...
///////////////////////////////////////////////////////////////////////////New code/////////////////////////////////////////////////////////////////////////////////////////////				
		...
		function save() {
			...
			if(!empty($_REQUEST['saveandpublish']))
			$tripPublishStatus = 1;
			...
			$status = $tripPublishStatus;
			
			
			$boat_price_range = $_REQUEST['boat_range'];
			 			
			
			/*session_start();
			$_SESSION['error'] = 'The boat you are changing is busy on other trips. Make sure to look at your trip schedules of this boat.';
			header('location:index.php?control=trip&task=addnew&id='.$_REQUEST['id']);
			die;*/
			...
			if(!$_REQUEST['id'] and ($boat_id!='')) {	
				...
				$query = "INSERT INTO `trips` ( `boat_id`, `trip_type_id`, `boat_price_range`, `country_id`, `destination_country_id`, `origin_id`, `destination_id`, `start_date`, `end_date`, `no_of_days`, `no_of_nights`, `no_of_dives`, `trip_price`, `price_for_diver`, `create_date`, `modified_date`, `status`) VALUES ('".$boat_id."', '".$trip_type_id."', '".$boat_price_range."', '".$country_id."', '".$destination_country_id."', '".$origin_id."', '".$destination_id."', '".$start_date."', '".$end_date."', '".$no_of_days."', '".$no_of_nights."', '".$no_of_dives."', '".$trip_price."', '".$price_for_diver."', '".$create_date."', '".$modified_date."', '".$status."')";
				...
				foreach($fuel_tanks as $tank){
					...
					$gastype = $_REQUEST['gastype'.$tank['id']];
					...
					$fuelprice = 0;
					if($fuel_tankId){
						$query = "INSERT INTO `trip_fuel_tank` ( `trip_id`, `fuel_tank_id`, `gastype`, `tank_price`, `status`) VALUES ('".$trip_id."', '".$fuel_tankId."', '".$gastype."', '".$fuelprice."', '1')";
						$this->Query($query);
						$this->Execute();
					}
				}
				...
				/*Getting Gases*/
				/*$query_gas_type ="SELECT * FROM  gas_type WHERE 1";
				$this->Query($query_gas_type);
				$gas_types = $this->fetchArray();
				foreach($gas_types as $gas_type){
					$gas_typeId = $_REQUEST['gas_type'.$gas_type['id']];
					if($gas_typeId){
						$query = "INSERT INTO `trip_gas_type` ( `trip_id`, `gas_type_id`, `status`) VALUES ('".$trip_id."', '".$gas_typeId."', '1')";
						$this->Query($query);
						$this->Execute();
					}
				}*/
				...
				if($typeOK && $image['name']){
				 ...
				 $query = "UPDATE `trips` SET `boat_id` = '".$boat_id."', `trip_type_id` = '".$trip_type_id."', `boat_price_range` = '".$boat_price_range."', `country_id` = '".$country_id."', `destination_country_id` = '".$destination_country_id."', `origin_id` = '".$origin_id."', `destination_id` = '".$destination_id."', `start_date` = '".$start_date."', `end_date` = '".$end_date."', `no_of_dives` ='".$no_of_dives."', `no_of_days` = '".$no_of_days."', `no_of_nights` = '".$no_of_nights."', `trip_price` = '".$trip_price."', `price_for_diver` = '".$price_for_diver."', `modified_date` = '".$modified_date."' WHERE id = '".$currentID."'";
				$this->Query($query);
				$this->Execute();
				...
				$delQuery = "DELETE FROM trip_fuel_tank  WHERE `trip_id` = '".$trip_id."'";
				$this->Query($delQuery);
				$this->Execute();
				...
				/*$delQuery = "DELETE FROM trip_gas_type  WHERE `trip_id` = '".$trip_id."'";
				$this->Query($delQuery);
				$this->Execute();
				...
				foreach($gas_types as $gas_type){
					$gas_typeId = $_REQUEST['gas_type'.$gas_type['id']];
					if($gas_typeId){
						$Gfetch = "SELECT * FROM  `trip_gas_type` WHERE gas_type_id = '".$gas_type['id']."' AND `trip_id` = '".$trip_id."'";
						$this->Query($Gfetch);
						$Gsfuel_tanks = $this->fetchArray();
							$query = "INSERT INTO `trip_gas_type` ( `trip_id`, `gas_type_id` , `status`) VALUES ('".$trip_id."', '".$gas_typeId."', '1')";
							$this->Query($query);
							$this->Execute();
					}
		...
		function addschedule() {
			...
			/*Getting trip Close*/
				...
				/*GET BOAT PRICE BECAUSE THE BOAT PRICE WILL BE THE TRIP / SCHEDULE PRICE */
				$query = "SELECT b.boat_price FROM boats b JOIN trips t ON t.boat_id = b.id  WHERE t.id = '".$_REQUEST['trip_id']."' ";
				$this->Query($query);
				$results = $this->fetchArray();
				$schedules[0]['trip_price'] = $results[0]['boat_price'];
				/*GET BOAT PRICE BECAUSE THE BOAT PRICE WILL BE THE TRIP / SCHEDULE PRICE */
			...
			if($_REQUEST['scheduleId']) {
				...
				/*CHECK IF THIS TRIP IS BOOKED BY CUSTOMERS OR NOT IF FOUND THEN NOTICE THE ADMIN AND SEND EMAIL REGARDING CHANGES ON TRIP TO ALL CUSTOMERS*/	
			 					...
			 					$isBookedQuery = "SELECT b.id
									...
									JOIN trip_schedules sc ON sc.id = b.trip_schedule_id
									WHERE sc.start_trip_datetime >=  '".date("Y-m-d")."'
									AND b.trip_schedule_id =  '".$_REQUEST['scheduleId']."'";
		...
		function scheduleSave() {
			...
			if($_REQUEST['trip_id']) {
				...
				$this->Query($query);
				$results = $this->fetchArray();
				$_REQUEST['trip_price'] = $results[0]['boat_price'];
				/*GET BOAT PRICE BECAUSE THE BOAT PRICE WILL BE THE TRIP / SCHEDULE PRICE */
		...
		function scheduleDelete() {
			...
			if($scheduleId) {
				...
				if(!$this->Execute())
					$check_transaction=="No";
					
				$q_delete_food = "DELETE FROM `trip_foods` WHERE trip_schedule_id = '".$scheduleId."'";
				$this->Query($q_delete_food);
				if(!$this->Execute())
					$check_transaction=="No";
					
				$q_delete_beverage = "DELETE FROM `trip_beverages` WHERE trip_schedule_id = '".$scheduleId."'";
				$this->Query($q_delete_beverage);
				if(!$this->Execute())
					$check_transaction=="No";
					
				$q_select_itinerary = "SELECT * FROM `trip_itinerary` WHERE id = '".$scheduleId."'";
				$this->Query($q_select_itinerary);
				$data_itinerarys = $this->fetchArray();
				foreach($data_itinerarys as $data_itinerary) {
					$q_delete_schedules = "DELETE FROM `trip_itinerary_details` WHERE trip_itinerary_id = '".$data_itinerary['id']."'";
				...
				$q_delete_itinerary = "DELETE FROM `trip_itinerary` WHERE id = '".$scheduleId."'";
				$this->Query($q_delete_itinerary);
				if(!$this->Execute())
					$check_transaction=="No";
				...
				if($check_transaction=="Yes")
				mysql_query("commit;");
				else
				mysql_query("rollback;");
			}
			
			header("location:index.php?control=trip&task=schedule&trip_id=".$tripId);
...
('".$itineraryId."', '".$lang['id']."', '".$_REQUEST["night_schedule".$lang['id']]."', '".$_REQUEST["detail".$lang['id']]."', 1)";
		...
		*/
		function updateNextItinerary($itineraryId=NULL, $scheduleID=NULL) {
			...
			if($itineraryId) {
				...
				$query = "SELECT ts.start_trip_datetime start_date, tit.id FROM trip_schedules ts JOIN trip_itinerary tit ON ts.id = tit.trip_schedule_id  WHERE ts.id = '".$scheduleID."' and tit.id = (select min(tit1.id) from trip_itinerary tit1 where tit1.trip_schedule_id = '".$scheduleID."')";
				
				$this->Query($query);
				$scheduleDate = $this->fetchArray();
				
				$q_update_first_itinerary = "UPDATE trip_itinerary SET itinerary_date = '".$scheduleDate[0]['start_date']."' WHERE id = '".$scheduleDate[0]['id']."'";
				$this->Query($q_update_first_itinerary);
				$this->Execute();
				
				$query = "SELECT * FROM trip_itinerary WHERE id = '".$scheduleDate[0]['id']."' and trip_schedule_id = '".$scheduleID."'";
			}
			$this->Query($query);
			...
			foreach($allData as $data) {
				...
				$dH = $dHours = $stay_hour + $arrivalHour + $dAddFormatData + $travel_hour + $extHour;
				...
				$aH = $aHour = $aHours + $aAddFormatData;
				...
				$itinerary_date = date("Y-m-d",strtotime($itinerary_date)+(($dAddDays*86400)));
				...
				$data_schedule_end_date = $this->fetchArray();
				$schedule_end_date = $data_schedule_end_date[0]['end_trip_datetime'];
			$str1 .= "ID=".$data['id'].": aHour=".$aH." dHour=".$dH."  ".$itinerary_date."<=".$schedule_end_date."   ";
		...
		/*	if(!$check_transaction) 
			...
			mysql_query("commit;");*/
			file_put_contents("rollback_status.txt",$str1);
		...
		*/
		...
		function saveOldItinerary($myTripId=NULL,$chooseOldScheduleId=NULL,$setToScheduleId=NULL) {
			
			$chooseScheduleId = $chooseOldScheduleId?$chooseOldScheduleId:$_REQUEST['chooseScheduleId'];
			$tripId = $myTripId?$myTripId:$_REQUEST['tripId'];
			$scheduleId = $setToScheduleId?$setToScheduleId:$_REQUEST['scheduleId'];
			...
			foreach($data as $result) {
				...
				$query = "INSERT INTO `trip_itinerary` (`trip_id`, `trip_schedule_id`, `deprture_country_id`, `arrival_country_id`, `departure_place`, `arrival_place`, `departure_time`, `arrival_time`, `travel_time`,`stay_hour`, `no_of_dive`, `status`) VALUES ( '".$tripId."', '".$scheduleId."', '".$result['deprture_country_id']."', '".$result['arrival_country_id']."', '".$result['departure_place']."', '".$result['arrival_place']."', '".$result['departure_time']."', '".$result['arrival_time']."', '".$result['travel_time']."', '".$result['stay_hour']."',  '".$result['no_of_dive']."', '1')";
				$this->Query($query);
				if($this->Execute()) {
			...
			////////////////////////////////////////////////////////////////
			$this->updateNextItinerary('', $scheduleId);
			/////////////////////////////////////////////////////////////////	
			if($chooseOldScheduleId)
			header("location:index.php?control=trip&task=itinerary&tripId=".$tripId."&scheduleId=".$scheduleId);
			else
			return true;
		}
		...
		function active() {
			if($_REQUEST['status']!='') {
				$query = "UPDATE trips SET status ='".$_REQUEST['status']."' WHERE id = ".$_REQUEST['id'];
				$this->Query($query);
				$this->Execute();	
				$this->show();
				//header("location:index.php?control=trip");
		...
		function saveCopySchedule() {
			$scheduleId = $_REQUEST['scheduleId'];
			/*GET COPY SCHEDULE DATA AND SAVE IT AS NEW SCHEDULE CODE START*/
			$query = "SELECT * FROM trip_schedules WHERE id = '".$scheduleId."'";
			$this->Query($query);
			$data_schedule = $this->fetchArray();
			
			$tripId = $data_schedule[0]['trip_id'];
			$start_trip_datetime = $data_schedule[0]['start_trip_datetime'];
			$end_trip_datetime = $data_schedule[0]['end_trip_datetime'];
			$trip_price = $data_schedule[0]['trip_price'];
			
			$scheduleStart = $_REQUEST['scheduleStart'];
			$scheduleEnd = date("Y-m-d",(strtotime($scheduleStart) + (strtotime($end_trip_datetime)-strtotime($start_trip_datetime))));
			
			$q_add_schedule = "INSERT INTO `trip_schedules` (`trip_id`, `start_trip_datetime`, `end_trip_datetime`, `trip_price`, `last_update`, `status`) VALUES
('".$tripId."', '".$scheduleStart."', '".$scheduleEnd."', '".$trip_price."', '".date("Y-m-d H:i:s")."', '1')";
			$this->Query($q_add_schedule);
			$this->Execute();
			$schedule_id = mysql_insert_id();
			/*GET COPY SCHEDULE DATA AND SAVE IT AS NEW SCHEDULE CODE END*/
			/*GET AND SET BEVERAGES FOR THE SCHEDULE START*/
			$q_beverage = "SELECT * FROM trip_beverages WHERE trip_schedule_id = '".$scheduleId."'";
			$this->Query($q_beverage);
			$data_beverages = $this->fetchArray();
			
			if(count($data_beverages)) {
				foreach($data_beverages as $data_beverage) {
					$str_beverage .= $str_beverage?",('".$tripId."', '".$schedule_id."', '".$data_beverage['beverage_id']."', '".$data_beverage['eq_value']."', '".$data_beverage['eq_type']."', '1')":"('".$tripId."', '".$schedule_id."', '".$data_beverage['beverage_id']."', '".$data_beverage['eq_value']."', '".$data_beverage['eq_type']."', '1')";
				}
				$q_insert_beverage = "INSERT INTO `trip_beverages` (`trip_id`, `trip_schedule_id`, `beverage_id`, `eq_value`, `eq_type`, `eq_status`) VALUES ".$str_beverage;
				$this->Query($q_insert_beverage);
				$this->Execute();
			}
			/*GET AND SET BEVERAGES FOR THE SCHEDULE END*/
			/*GET AND SET EQUIPMENTS FOR THE SCHEDULE START*/
			$q_equipment = "SELECT * FROM trip_equipments WHERE trip_schedule_id = '".$scheduleId."'";
			$this->Query($q_equipment);
			$data_equipments = $this->fetchArray();
			if(count($data_equipments)) {
				foreach($data_equipments as $data_equipment) {
					$str_equipment .= $str_equipment?",('".$tripId."', '".$schedule_id."', '".$data_equipment['equipment_id']."', '".$data_equipment['eq_value']."', '".$data_equipment['eq_type']."', '1')":"('".$tripId."', '".$schedule_id."', '".$data_equipment['equipment_id']."', '".$data_equipment['eq_value']."', '".$data_equipment['eq_type']."', '1')";
				}
				$q_insert_equipment = "INSERT INTO `trip_equipments` (`trip_id`, `trip_schedule_id`, `equipment_id`, `eq_value`, `eq_type`, `eq_status`) VALUES ".$str_equipment;
				$this->Query($q_insert_equipment);
				$this->Execute();
			}
			/*GET AND SET EQUIPMENTS FOR THE SCHEDULE END*/
			/*GET AND SET FOODS FOR THE SCHEDULE START*/
			$q_foods = "SELECT * FROM trip_foods WHERE trip_schedule_id = '".$scheduleId."'";
			$this->Query($q_foods);
			$data_foods = $this->fetchArray();
			if(count($data_foods)) {
				foreach($data_foods as $data_food) {
					$str_food .= $str_food?",('".$tripId."', '".$schedule_id."', '".$data_food['food_id']."', '".$data_food['eq_value']."', '".$data_food['eq_type']."', '1')":"('".$tripId."', '".$schedule_id."', '".$data_beverage['beverage_id']."', '".$data_beverage['eq_value']."', '".$data_beverage['eq_type']."', '1')";
				}
				$q_insert_food = "INSERT INTO `trip_foods` (`trip_id`, `trip_schedule_id`, `food_id`, `eq_value`, `eq_type`, `eq_status`) VALUES ".$str_food;
				$this->Query($q_insert_food);
				$this->Execute();
			}
			/*GET AND SET FOODS FOR THE SCHEDULE END*/
			
			/*SET ITINERARY START*/
			$chooseScheduleId = $scheduleId;
			$tripId = $tripId;
			$scheduleId = $schedule_id;
			$this->saveOldItinerary($tripId,$chooseScheduleId,$scheduleId);
