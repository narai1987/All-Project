  ...
  <div class="detail_container">
    <div class="head_cont">
      <h2 class="main_head">
        <table width="99%">
          <tr>
            <td width="50%" class="main_txt">State</td>
            <td align="center"><form name="filterForm" id="filterForm" method="post" action="index.php" >
                ...
                <input type="hidden" name="defftask" id="defftask" value="state"  />
                <input type="hidden" name="del" id="del"  />
                <input type="hidden" name="edit" id="edit"  />
              </form></td>
            <td><a href="index.php?control=countrie&task=state_addnew" ><img src="images/add_new.png" /></a></td>
          </tr>
        </table>
      </h2>
    </div>
    <div class="grid_container" id="mmm">
      <table width="99%" cellpadding="0" cellspacing="0" class="grid">
        <tr>
          ...
          <td class="tr_haed bord_right" width="35%"><strong>State</strong></td>
...
?>
        <tr class="<?php echo $class; ?>">
          ...
          <td ><?php  $sql = "select * from states s join countries c ON s.country_id = c.id where s.country_id='".$result['country_id']."' and s.language_id = ".$_SESSION['language_id'];
      $cun = mysql_query($sql);
	 $data = mysql_fetch_array($cun); 

echo $data['country'];  ?></td>
          <td ><?php echo $result['state']; ?></td>
          <td  align="center"><a onclick="state_edit(<?php echo $result['id']; ?>)" style="cursor:pointer;" title="Edit"><img src="images/edit.png" alt="edit" title="Edit" /></a>
            <?php if($result['status']){  ?>
            
            <!--<a href="index.php?control=countrie&task=state_status&id=<?php echo $result['id'] ; ?>&status=0" title="Click to Unpublish" >--> 
            <a onclick="state_status(<?php echo $result['id']; ?>,0)" style="cursor:pointer;" title="Click to Unpublish"> <img src="images/backend/check.png" alt="check_de" /></a>
            <?php } else { ?>
            
            <!--<a href="index.php?control=countrie&task=state_status&id=<?php echo $result['id'] ; ?>&status=1" title="Click to Publish" >--> 
            <a onclick="state_status(<?php echo $result['id']; ?>,1)" style="cursor:pointer;" title="Click to Publish"> <img src="images/backend/check_de.png" alt="check_de" /></a>
            <?php } ?>
            <a onclick="states_delete(<?php echo $result['id']; ?>)" title="Delete" ><img src="images/backend/del.png" alt="Delete" title="Delete" /></a></td>
        </tr>
        <?php } }else{ ?>
        <tr>
