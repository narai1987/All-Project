...
<div class="detail_container">
    <div class="head_cont">
      <h2 class="main_head">
        <table width="99%">
          <tr>
            <td width="55%" class="main_txt" >Site Languages</td>
            ...
            <td align="right"><a href="index.php?control=language&task=addnew" class="button" ><!--onclick="addLanguage()"--> 
              <img src="images/add_new.png"></a></td>
          </tr>
        </table>
      </h2>
    </div>
    <div class="grid_container">
      ...
      <table width="99%" cellpadding="0" cellspacing="0" class="grid">
        <tr>
          <td class="tr_haed" width="3%" align="center" ><strong>Sr.no.</strong></td>
          <td class="tr_haed bord_right" width="20%"><strong>Languages</strong></td>
          <td class="tr_haed bord_right" width="3%"><strong><?php echo $content['action'];?></strong></td>
        </tr>
        <?php
	if($results) {
	$i = (($page-1)*$tpages);
...
?>
        <tr class="<?php echo $class; ?>">
          ...
          <td  ><!--<a href="index.php?control=language&task=addnew&id=<?php echo $result['id'] ; ?>" class="button"> -->
          <a onclick="language_edit(<?php echo $result['id']; ?>)" style="cursor:pointer;" title="Edit">
          <img src="images/backend/edit.png" alt="setting_grd" title="Edit" /></a>
            <?php if($result['id']!='1'){ ?>
            <?php if($result['status']){  ?>
            <a href="index.php?control=language&task=status&id=<?php echo $result['id'] ; ?>&page=<?php echo $_REQUEST['page']; ?>&tpages=<?php echo $_REQUEST['tpages']; ?>&status=0" title="Click to publish" ><img src="images/backend/check.png" alt="check_de" /></a>
            <?php }else{ ?>
            <a href="index.php?control=language&task=status&id=<?php echo $result['id'] ; ?>&page=<?php echo $_REQUEST['page']; ?>&tpages=<?php echo $_REQUEST['tpages']; ?>&status=1" title="Click to unpublish" ><img src="images/backend/check_de.png" alt="check_de" /></a>
            <?php } ?>
            <?php } ?>
            <?php if($result['id']!='1'){ ?>
            <a href="index.php?control=language&task=delete&id=<?php echo $result['id']; ?>&page=<?php echo $_REQUEST['page']; ?>&tpages=<?php echo $_REQUEST['tpages']; ?>"  onclick="return confirm('Are you sure you want to delete ?');" title="Delete" ><img src="images/backend/del.png" alt="Delete" /></a>
            <?php } ?></td>
        </tr>
        <?php }
	} else{?>
...
document.getElementById("logindivid").innerHTML ='<p align="center" style="margin:50px 50px 50px 50px;" ><img src="images/ajax-loader.gif" /></p>';
		...
		//alert("gjh kh kjh");
		xmlhttp.open("GET","script/language/add_language.php",true);
		xmlhttp.send();
...
document.getElementById("logindivid").innerHTML ='<p align="center" style="margin:50px 50px 50px 50px;" ><img src="images/ajax-loader.gif" /></p>';
		...
		//alert(id);
		xmlhttp.open("GET","script/language/add_language.php?id="+id,true);
		xmlhttp.send();
}
...
function validation_language(id)
{ 
	var cnt = 0;
	...
	if(document.getElementById('content').value=='') {
		...
		document.getElementById('msgcontent').innerHTML="Invalid language";
	}
