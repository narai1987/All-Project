  ...
  <div class="detail_container">
    <div class="head_cont">
      <h2 class="main_head">
        <table width="99%">
          <tr>
            <td width="60%" class="main_txt">Foods
            ...
            <td><a href="index.php?control=food&task=addnew" ><img src="images/add_new.png" /></a></td>
          </tr>
        </table>
      </h2>
    </div>
    <div class="grid_container" id="mmm">
      <table width="99%" cellpadding="0" cellspacing="0" class="grid">
        <tr>
          ...
          <td class="tr_haed" width="15%"><strong>Food Types</strong></td>
          <td class="tr_haed" width="35%"><strong>Food </strong></td>
...
?>
        <tr class="<?php echo $class; ?>">
          ...
          <td><?php  $sql = "select * from foods f join food_types ft ON f.food_type_id  = ft.id where f.food_type_id='".$result['food_type_id']."' and f.language_id = ".$_SESSION['language_id'];
      $cun = mysql_query($sql);
	 $data = mysql_fetch_array($cun); 

echo $data['food_type'];  ?></td>
          <td><?php echo $result['food']; ?></td>
          <td align="center"><a onclick="food_edit(<?php echo $result['id']; ?>)" style="cursor:pointer;" title="Edit"><img src="images/edit.png" alt="edit" title="Edit" /></a>
            <?php if($result['status']){  ?>
            <a onclick="food_status(<?php echo $result['id']; ?>,0)" style="cursor:pointer;" title="Click to Unpublish"><img src="images/backend/check.png" alt="check_de" /></a>
            <?php } else { ?>
           <a onclick="food_status(<?php echo $result['id']; ?>,1)" style="cursor:pointer;" title="Click to Publish"><img src="images/backend/check_de.png" alt="check_de" /></a>
            <?php } ?>
            <a onclick="food_delete(<?php echo $result['id']; ?>)" style="cursor:pointer;" title="Delete"><img src="images/backend/del.png" alt="Delete" title="Delete" /></a></td>
        </tr>
        <?php } }else{ ?>
        <tr>
